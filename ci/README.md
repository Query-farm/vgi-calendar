...
4. **Run** — [`run-integration.sh`](run-integration.sh) stages the preprocessed
   tree, resolves `VGI_CALENDAR_WORKER` (the ATTACH `LOCATION`) per the
...
| `TRANSPORT`  | `VGI_CALENDAR_WORKER` (LOCATION)     | How the worker is reached |
|--------------|--------------------------------------|---------------------------|
| `subprocess` | `uv run … calendar_worker.py`        | extension spawns the worker per query; Arrow IPC over stdin/stdout (default) |
| `http`       | `http://127.0.0.1:<port>`            | harness boots `calendar_worker.py --http --port 0 --port-file <f>`, waits for the port-file, then ATTACHes that URL |
| `unix`       | `unix:///tmp/cal-<pid>.sock`         | harness boots `calendar_worker.py --unix <sock>`, waits for the socket to appear, then ATTACHes it |
...
### Per-transport status (reference pilot)

- **subprocess**: GREEN — 94 assertions.
- **http**: GREEN — 102 assertions (94 + the injected httpfs INSTALL/LOAD
...
- **unix**: GREEN — 94 assertions. No extra deps; `--unix` is built into the
...
WORKER_CMD="uv run --python 3.13 calendar_worker.py" \
...
`uv run --python 3.13 <repo>/calendar_worker.py`, so a bare

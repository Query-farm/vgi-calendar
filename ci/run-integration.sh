...
#
# Run this repo's sqllogictest suite (test/sql/*.test) against the vgi-calendar
...
# ATTACH (`${VGI_CALENDAR_WORKER}`):
#
#   subprocess : a bare stdio command (`uv run calendar_worker.py`) — the
...
#                        `uv run --python 3.13 <repo>/calendar_worker.py`.
# Optional:
...
WORKER_CMD="${WORKER_CMD:-uv run --python 3.13 $REPO/calendar_worker.py}"
...
# ---------------------------------------------------------------------------
# Per-transport: resolve VGI_CALENDAR_WORKER (the LOCATION) and, for the
...
case "$TRANSPORT" in
  subprocess)
    export VGI_CALENDAR_WORKER="$WORKER_CMD"
    ;;
  ...
  http)
    ...
    PORT_FILE="$(mktemp -u "${TMPDIR:-/tmp}/cal-port.XXXXXX")"
    LOG_FILE="${TMPDIR:-/tmp}/cal-http-server.log"
    ...
    export VGI_CALENDAR_WORKER="http://127.0.0.1:$PORT"
    ;;
  ...
  unix)
    ...
    SOCK="${TMPDIR:-/tmp}/cal-$$.sock"
    rm -f "$SOCK"
    LOG_FILE="${TMPDIR:-/tmp}/cal-unix-server.log"
    ...
    export VGI_CALENDAR_WORKER="unix://$SOCK"
    ;;
...
echo "Running suite (transport: $TRANSPORT, worker: $VGI_CALENDAR_WORKER) ..."

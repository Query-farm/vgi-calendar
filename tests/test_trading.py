"""Unit + edge-case tests for the trading / exchange-calendar layer.

These exercise the pure :mod:`vgi_calendar.trading` math directly (no Arrow /
RPC) plus the broadened holiday coverage in :mod:`vgi_calendar.core`. The
end-to-end ATTACH+SELECT path is covered by ``test/sql/trading.test``.
"""
...
from vgi_calendar import core, trading

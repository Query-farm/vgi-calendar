# vgi-calendar — dev and test targets.
#
# Usage:
...
#
# test-sql is self-contained: it points VGI_CALENDAR_WORKER at the worker run as
...
WORKER_STDIO    ?= uv run --python 3.13 calendar_worker.py
...
test-sql:
	PATH="$(HAYBARN_BIN):$$PATH" \
		VGI_CALENDAR_WORKER="$(WORKER_STDIO)" \
